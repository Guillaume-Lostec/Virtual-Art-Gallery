...
class GlitchPass extends Pass {
	...
	render( renderer, writeBuffer, readBuffer ) { // Replaced the previous one so the glitch is on all the time

		this.uniforms[ 'tDiffuse' ].value = readBuffer.texture;
		this.uniforms[ 'seed' ].value = Math.random()*0.1;
		this.uniforms[ 'byp' ].value = 0; // never bypass

		// Always-on, subtle glitch
		this.uniforms[ 'amount' ].value = 0.001 + Math.random() * 0.001; // intensity
		this.uniforms[ 'angle' ].value = MathUtils.randFloat( - Math.PI, Math.PI );
		this.uniforms[ 'seed_x' ].value = MathUtils.randFloat( - 0.3, 0.3 );
		this.uniforms[ 'seed_y' ].value = MathUtils.randFloat( - 0.3, 0.3 );
		this.uniforms[ 'distortion_x' ].value = MathUtils.randFloat( 0.2, 0.6 );
		this.uniforms[ 'distortion_y' ].value = MathUtils.randFloat( 0.2, 0.6 );

		// Optional: make intensity vary over time (slow oscillation)
		this.uniforms[ 'amount' ].value *= 0.5 + 0.5 * Math.sin(performance.now() * 0.001);
